...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white border-b sticky top-16 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex gap-8">
            ...
            <a href="?tab=purchase-history" class="py-4 font-medium border-b-2 <?php echo $tab == "purchase-history" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-600 hover:text-gray-900"; ?>">
                Purchase History
            </a>
        </div>
    </div>
    ...
    <?php elseif ($tab == "cart"): ?>
        ...
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <?php if (empty($cartItems)): ?>
                <div class="bg-white rounded-2xl border border-slate-100 p-16 text-center shadow-sm">
                    ...
                    <a href="search-products.php" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-all shadow active:scale-95">Browse Skills</a>
                </div>
            <?php else: ?>
                            ...
                            <div onclick='openProductModal(<?= json_encode($productData) ?>);' id="cart-item-<?= $item["cart_item_id"]; ?>"
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-start">
                                        ...
                                        <button onclick="removeItem(<?= $item['cart_item_id'] ?>, event);" class="text-slate-300 hover:text-rose-500 p-1 transition-colors" title="Remove">🗑️</button>
                                    </div>
        ...
        <!-- Toast -->
        <div id="toast" style="position:fixed; top:20px; right:20px; z-index:9999; transform:translateY(-120px); transition:transform 0.3s ease; pointer-events:none;">
        ...
        <div class="fixed inset-0 z-[100] hidden" id="productModal">
            ...
            <div id="modalContent" class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-11/12 max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row transform scale-95 opacity-0 transition-all duration-300">
                <div class="md:w-1/2 relative bg-slate-100 aspect-video md:aspect-auto">
                    ...
                    <span id="modalLevel" class="absolute top-4 left-4 text-xs font-bold px-3 py-1 rounded-md bg-blue-600 text-white uppercase tracking-wider shadow-md">Beginner</span>
                </div>
                <div class="p-8 md:w-1/2 flex flex-col relative">
                    <button onclick="closeProductModal();" class="absolute top-5 right-5 text-slate-400 hover:text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-full w-10 h-10 flex items-center justify-center transition-colors">X</button>
                    <h3 id="modalTitle" class="text-3xl font-extrabold text-gray-900 mb-3 leading-tight">Course Title</h3>
                    <p id="modalSeller" class="text-sm text-slate-600 mb-6">By Seller Name</p>
                    <p id="modalDesc" class="text-base text-slate-700 mb-7 flex-1 overflow-y-auto max-h-56 leading-relaxed">Description Here..</p>
        ...
        <?php if (!empty($cartItems)): ?>
            ...
            <script>
                ...
                async function removeItem(cartItemID, e) {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            if (cc) cc.textContent = data.itemCount;
                ...
                async function checkout() {
                    try {
                        const res = await fetch("Process/payherePorcess.php", {
                            method: "POST"
                        });
                        const data = await res.json();
                        if (data.success) {
            ...
            <div class="grid md:grid-cols-3 gap-6 h-[600px] border border-gray-100 rounded-3xl overflow-hidden shadow-sm bg-white">
                ...
                <div class="flex flex-col border-r border-gray-100 h-[600px] overflow-hidden">
                    <div class="p-6 border-b border-gray-100 flex-shrink-0">
                        <input type="text" id="chatSearch" onkeyup="filterChats()" placeholder="search...." class="w-full px-4 py-2.5 bg-gray-50 
                        border-none rounded-xl text-sm focus:ring-4 focus:ring-blue-50/50
                ...
                <div class="md:col-span-2 flex flex-col bg-gray-50/30 h-[600px] overflow-hidden">
                    ...
                    <div id="messageArea" class="overflow-y-auto flex-1 p-6 space-7 min-h-0">
...
`;
        ...
        ?>
                            ...
                            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                ...
                                <div class="divide-y divide-gray-100">
                                    ...
                                    $itemsQ = Database::search(
                                        "SELECT ii.*, p.`title`, p.`image_url`, u.`fname` AS `seller_fname`, u.`lname` AS `seller_last`,
                                    (SELECT `id` FROM `feedback` WHERE `user_id`=? AND `product_id`=ii.`product_id` LIMIT 1) AS `feedback_id`
                                    ...
                                    ?>
                                        ...
                                        <a href="product-view.php?id=<?= $item["product_id"] ?>"
                                            class="p-6 flex flex-col md:flex-row gap-6 items-start md:items-center hover:bg-blue-50 transition-colors">
                                            ...
                                            <div class="flex-grow">
                                                ...
                                                <p class="text-sm text-gray-500 mt-1">Sold By <span class="font-medium text-gray-700"><?= $item["seller_fname"] . " " . $item["seller_last"]; ?></span></p>
            ...
            <div id="feedback-modal" class="fixed inset-0 z-[100] hidden">
                <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" onclick="closeFeedbackModal();">
                </div>
                            ...
                            <form id="feedback-form" onsubmit="submitFeedback(event);">
                                ...
                                <div class="mb-6">
                                    ...
                                    <div class="flex gap-2">
                                        <?php for ($i = 1; $i <= 5; $i++): ?>
                                            <button type="button" onclick="setRating(<?= $i ?>);" class="rating-star text-3xl text-gray-300 hover:text-yellow-400 transition-colors" data-value="<?= $i ?>">☆</button>
                                        <?php endfor; ?>
                                ...
                                <div class="mb-8">
                                    ...
                                    <textarea id="modal-message" rows="4" required placeholder="Share your thoughts about this skill.."
                                        class="w-full px-4 py-3 rounded-2xl bg-gray-50 border border-transparent focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all outline-none text-gray-700 resize-none"></textarea>
                                </div>

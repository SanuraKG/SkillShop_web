...
$p = Database::search(
    "SELECT p.*, c.`name` AS `cat_name`, u.`fname`, u.`lname`, u.`id` AS `sid`, u.`created_at` AS `s_joined`
    FROM `product` p
...
// Reviews — fetch fname and lname for reviewer name + initials
...
$reviews = [];
while ($row = $revQ?->fetch_assoc()) $reviews[] = $row;

// Seller stats: course count, student count, avg rating
$ss = Database::search(
...
// Related products by the same seller (exclude current)
...
$related = false;
while ($row = $relQ->fetch_assoc()) $related[] = $row;
...
<div class="min-h-screen" style="background:#f8fafc;">
    ...
    <!-- Hero Banner -->
    <div class="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-500 to-blue-900" style="min-height:280px;">
        <?php if ($p["image_url"]): ?>
            <img src="<?= $p["image_url"] ?>" class="absolute inset-0 w-full h-full object-cover opacity-10 scale-105 blur-sm pointer-events-none">
        <?php endif; ?>
        ...
        <div class="relative max-w-6xl mx-auto px-4 pt-10 pb-16">
            ...
            <nav class="flex items-center gap-2 text-sm text-slate-400 mb-6">
                ...
                <?php if ($p["cat_name"]): ?>
                    <span>></span>
                    <a href="search-products.php?cat=<?= $p["category_id"] ?>" class="hover:text-white transition-colors"><?= $p["cat_name"] ?></a>
                <?php endif; ?>
            </nav>

            <!-- Badges: level, category, enrolled status -->
            <div class="flex flex-wrap gap-2 mb-4">
                ...
                <?php endif; ?>
                <?php if ($bought): ?>
                    <span class="inline-flex items-center text-xs font-semibold px-3 py-1 rounded-full bg-blue-500/25 text-green-300">✔️ Enrolled</span>
                <?php endif; ?>
            </div>

            <!-- Product title from DB -->
    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="flex-1 min-w-0 space-y-6">
                    ...
                    <?php if ($reviews): ?>
                        <div class="space-y-4">
                            <?php foreach ($reviews as $rv):
                                // Build reviewer display name, fallback to Anonymous
                                $name = trim(($rv["fname"] ?? "") . " " . ($rv["lname"] ?? "")) ?: "Anonymous";
                                // Build initials for avatar circle
                                $init = strtoupper($rv["fname"][0] ?? "A") . strtoupper($rv["lname"][0] ?? "");
                            ?>
                                <!-- Single review card -->
                                <div class="flex gap-3 p-4 rounded-2xl bg-slate-50">
                                    <!-- Initials avatar -->
                                    <div class="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center text-white text-xs font-bold bg-gradient-to-br from-blue-500 to-indigo-600"><?= $init ?></div>
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <!-- Stars filled by JS via data-star -->
                ...
                <!-- Related skills by same seller -->
                <?php if ($related): ?>
                    <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
                        <h2 class="text-lg font-bold text-gray-900 mb-4">More By <?= $sellerName ?></h2>
                        <!-- All related cards in one grid row -->
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            <?php foreach ($related as $rel): ?>
                                <a href="product-detail.php?id=<?= $rel["id"] ?>" class="group block rounded-xl overflow-hidden border border-slate-100 hover:border-blue-200 hover:shadow-md transition-all">
                                    <div class="h-28 bg-slate-200 overflow-hidden">
                                        <?php if ($rel["image_url"]): ?>
                                            <img src="<?= $rel["image_url"] ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                                        <?php endif; ?>
                                    </div>
                                    <div class="p-3">
                                        <p class="text-xs text-blue-600 font-semibold"><?= $rel["level"] ?></p>
                                        <p class="text-sm font-bold text-gray-900 group-hover:text-blue-500 transition-colors line-clamp-2"><?= $rel["title"] ?></p>
                                        <p class="text-sm font-bold text-blue-600 mt-1">Rs. <?= number_format($rel["price"], 2) ?></p>
                                    </div>
                                </a>
                            <?php endforeach; ?>
                        </div>
                ...
                <div class="sticky top-24 space-y-4">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-xl">
                        ...
                        <div class="p-5">
                            ...
                            <!-- Enroll/action buttons — shown based on user state -->
                            <?php if ($bought): ?>
                                <!-- User already enrolled -->
                                <div class="flex items-center justify-center gap-2 py-3 mb-3 rounded-xl text-sm font-bold text-green-700 bg-green-50 border-2 border-green-300">✔️ Already Enrolled</div>

                                <a href="buyer-dashboard.php?tab=learning" class="block w-full py-3.5 text-center bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white 
                                font-bold rounded-xl transition-all active:scale-95 mb-3">Go to my Learnings</a>
                            ...
                            <?php elseif (!$loggedIn): ?>
                                ...
                                <a href="index.php" class="block w-full py-3.5 text-center bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700
                                 hover:to-indigo-700 text-white font-bold rounded-xl transition-all active:scale-95 shadow-md mb-3">Sign In to Enroll</a>
                            ...
                            <div class="space-y-2.5 text-sm border-t border-slate-100 pt-4">
                                <?php foreach (
                                    [
                                        ...
                                        ["📅", "Added",    date("M Y", strtotime($p["created_at"]))],
                                    ] as [$ico, $lbl, $val]
                                ): ?>
                    ...
                    <!-- Instructor Card -->
                    <div class="bg-white rounded-2xl border border-slate-100 p-5 overflow-hidden shadow-xl">
                        <h3 class="font-bold text-gray-900 mb-4">Instructor</h3>
                        <div class="flex items-center gap-3 mb-4">
                            <!-- Single initial avatar -->
                            <div class="w-12 h-12 rounded-2xl flex-shrink-0 flex items-center justify-center text-white font-bold text-lg bg-gradient-to-br from-blue-600 to-indigo-600">
                                <?= strtoupper($p["fname"][0] ?? "S") ?>
                            </div>
                            <div>
                        ...
                        <!-- Instructor stats: courses, students, rating -->
                        <div class="grid grid-cols-3 gap-2 text-center mb-4">
                            ...
                            <div class="rounded-xl py-2 bg-blue-50">
                                <p class="font-extrabold text-sm text-blue-600"><?= number_format($ss["ts"]) ?></p>
                                <p class="text-xs text-gray-500">Students</p>
                            </div>
                            <div class="rounded-xl py-2 bg-blue-50">
                                <p class="font-extrabold text-sm text-blue-600"><?= $ss["tr"] > 0 ? round($ss["tr"], 1) : "-" ?></p>

...
//How many active product mtch users search
    ...
    <section class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-10 md:py-14">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p class="text-blue-100 text-center text-sm mt-6">
                <?php
                if ($totalProducts > 0) {
                    echo "<span class='font-bold'>" . $totalProducts . "</span> " . ($totalProducts == 1 ? "skill" : "skills") . " found";
        ...
        <?php if (count($products) > 0): ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                ...
                <?php foreach ($products as $product):
                    ...
                    <!-- Product Card -->
                    <div class="relative bg-white rounded-xl shadow hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col border border-gray-100 group">

                        <a href="product-view.php?id=<?php echo $product["id"]; ?>" class="absolute inset-0 z-10" aria-label="<?php echo $product["title"]; ?>"></a>
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <a href="seller-profile.php?id=<?php echo $product["seller_id"]; ?>" class="relative z-20 text-xs text-blue-600 hover:text-blue-700 font-medium mb-1.5 line-clamp-1 w-fit">
                                <?php echo $product["seller_name"]; ?>
                            </a>

                            <!-- Title -->
                            <h3 class="font-bold text-gray-900 text-sm leading-snug line-clamp-2 mb-2 group-hover:text-blue-600 transition-colors">
                            ...
                            <div class="flex items-center justify-between mt-auto">
                                ...
                                <div class="px-2.5 py-1.5 bg-blue-50 text-blue-600 text-xs font-bold rounded-lg group-hover:bg-blue-600 group-hover:text-white transition-colors">→</div>
                            </div>
        ...
        <nav class="flex justify-center items-center gap-1 mt-12 mb-8">
            ...
            <?php if ($currentPage > 1): ?>
                <a href="?q=<?php echo urlencode($searchQuery); ?>&page=1"
                    class="p-2 text-gray-600 hover:bg-blue-50 rounded-lg transition-colors" title="First">&laquo;</a>

                <a href="?q=<?php echo urlencode($searchQuery); ?>&page=<?php echo $currentPage - 1; ?>"
                    class="p-2 text-gray-600 hover:bg-blue-50 rounded-lg transition-colors" title="Previous">&lsaquo;</a>
            <?php endif; ?>
            ...
            <?php for ($i = $start; $i <= $end; $i++): ?>
                ...
                <a href="?q=<?php echo urlencode($searchQuery); ?>&page=<?php echo $i; ?>"
                    class="px-3 py-2 text-sm font-medium rounded-lg transition-colors <?php echo $i == $currentPage ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-blue-50'; ?>">
                    <?php echo $i; ?>
                </a>
            <?php endfor; ?>
            ...
            <?php if ($end < $totalPages): ?>
                ...
                <a href="?q=<?php echo urlencode($searchQuery); ?>&page=<?php echo $totalPages; ?>"
                    class="px-3 py-2 text-sm font-medium text-gray-700 hover:bg-blue-50 rounded-lg transition-colors">
                    <?php echo $totalPages; ?>
                </a>
            <?php endif; ?>
            ...
            <?php if ($currentPage < $totalPages): ?>
                <a href="?q=<?php echo urlencode($searchQuery); ?>&page=<?php echo $currentPage + 1; ?>"
                    class="p-2 text-gray-600 hover:bg-blue-50 rounded-lg transition-colors" title="Next">&rsaquo;</a>

                <a href="?q=<?php echo urlencode($searchQuery); ?>&page=<?php echo $totalPages; ?>"
                    class="p-2 text-gray-600 hover:bg-blue-50 rounded-lg transition-colors" title="Last">&raquo;</a>
            <?php endif; ?>

...
$q = Database::search(
    "SELECT 
    p.`id`, p.`title`, p.`image_url`, p.`level`, p.`price`, 
    p.`created_at`, u.`fname` AS `seller_name`, 
...
border-gray-200 text-gray-700 hover:border-blue-400 hover:text-blue-600 shadow-sm hover:shadow-md transition-all shrink-0">
        ...
        <?php if (count($items) > 0): ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" id="watchlist-grid">
                ...
                ?>
                    <article class="group relative bg-white/80 backdrop-blur rounded-2xl transition-all duration-300 hover:-translate-y-1" data-product-id="<?= $p["id"]; ?>">
                        <a href="product-view.php?id=<?= $p["id"]; ?>" class="block">
                            <div class="relative h-44 bg-gradient-to-br from-slate-100 to-slate-50 overflow-hidden rounded-t-2xl">
                                <?php if ($p["image_url"]) : ?>
                            ...
                            <div class="p-5">
                                ...
                                <div class="flex items-center justify-between mt-4">
                                    ...
                                    <div class="text-lg font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
                                        Rs. <?= number_format($p["price"], 2); ?></div>
                                </div>
                            </div>
                        </a>

                        <button type="button" class="wl-heart absolute top-3 right-3 z-10 w-10 h-10 flex items-center justify-center
                rounded-full bg-white/90 backdrop-blur shadow-md text-rose-500 hover:scale-110 active:scale-95 transition-transform"
        ...
        <?php else: ?>
            ...
            to-indigo-600 text-white font-bold rounded-xl hover:from-blue-700 hover:to-indigo-700 shadow-lg hover:shadow-xl transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                    Browse Skills
                ...
                try {
                    ...
                    if (j.success && j.action == "removed") {
                        ...
                        if (itemsCount == 0) {
                            ...
                            gridSection.innerHTML += `           <div class="relative bg-white/80 backdrop-blur rounded-3xl border border-dashed border-gray-200 p-16 text-center shadow-lg max-w-xl mx-auto">
                <div class="inline-flex w-24 h-24 items-center justify-center rounded-full bg-gradient-to-br from-rose-100 to-indigo-100 text-5xl mb-6">❤️</div>
                <h2 class="text-2xl font-bold text-gray-900 mb-2">Your Watchlist is empty</h2>
                <p class="text-gray-500 mb-8 max-w-sm mx-auto">Save skills you're interested in with the heart icon and find them here</p>
                <a href="search-products.php" class="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-blue-600
